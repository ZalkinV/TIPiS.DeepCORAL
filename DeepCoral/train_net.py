	...
	with tf.Graph().as_default():  
		...
		with tf.Session() as sess:
			...
			try:
				...
				while step < 5 and not coord.should_stop():
					...
					if step % 1 == 0:
						...
						format_str = ('%s: step %d, classification_loss = %.2f, coral_loss = %.2f, loss = %.2f (%.1f examples/sec; %.3f sec/batch)')
						print (format_str % (datetime.now(), step, cl_loss, cr_loss, loss_value, examples_per_sec, sec_per_batch))
					
					#if step % 5 == 0:
					#	dirname = os.path.dirname(__file__)
					#	dest_directory = os.path.join(dirname, DIR_TRAIN)
					#	checkpoint_name = os.path.join(dest_directory, 'model_epoch'+str(step / 100)+'.ckpt')
